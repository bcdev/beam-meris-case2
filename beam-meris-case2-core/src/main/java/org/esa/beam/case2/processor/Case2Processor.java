...
public abstract class Case2Processor extends Processor {
    ...
    protected Case2Processor(Case2ProcessorConstants constants) {
        this.constants = constants;
        logger = Logger.getLogger(constants.getProcessorLoggerName());
        setDefaultHelpId(constants.getProcessorHelpId());
    }

    protected abstract Class<? extends AlgorithmParameter> getParameterType();

    protected abstract MerisGLM getMerisGLM();

    protected abstract WaterAlgorithm getWaterAlgorithm();
    ...
    private void loadParameter(File parameterFile) throws ProcessorException {
        ...
        try {
            parameter = ObjectIO.readObject(getParameterType(), parameterFilePath);
        } catch (IOException e) {
    ...
    public void process(ProgressMonitor pm) throws ProcessorException {
        try {
            ...
            try {
                ...
                outputBands = algo.init(inputProduct, inputBandNames, getWaterAlgorithm(), getMerisGLM(),
                                        parameter, auxdata
                );
                if (pm.isCanceled()) {
                    return;
    ...
    /*
     * Loads the input product from the request.
     */
    ...
    /*
     * Performs the actual processing of the output product.
     */
